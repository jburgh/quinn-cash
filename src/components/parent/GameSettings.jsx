...
const DEFAULT_SETTINGS = { incorrectThreshold: 1, dailyCap: 5 }
...
export default function GameSettings() {
  ...
  const handleLaunchBonus = async () => {
    ...
    try {
      ...
      toast.success('Bonus session launched on the kid screen!')
    } catch {
      toast.error('Failed to launch bonus session')
    } finally {
      setLaunching(false)
    }
  }
  ...
  return (
    <div className="p-4 pb-8 border-t border-gray-100 mt-4">
      ...
      <p className="font-body text-gray-400 text-sm mb-6">
        Earn 1 Quinn Cash per correct word.
      </p>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm mb-6">
        <p className="font-body text-sm font-bold text-gray-700 mb-1">Daily cap</p>
        <p className="font-body text-xs text-gray-400 mb-3">
          Maximum Quinn Cash from spelling per day
        </p>
        <div className="flex items-center gap-4">
          <button
            onClick={() => update('dailyCap', settings.dailyCap - 1)}
            className="w-10 h-10 rounded-xl bg-gray-100 text-gray-600 font-display text-xl flex items-center justify-center active:scale-90 transition-transform"
          >
            −
          </button>
      ...
      >
        {launching ? 'Launching...' : '🚀 Launch Bonus Session'}
      </button>
      <p className="font-body text-xs text-gray-400 text-center mt-2">
        Opens the spelling game on the kid screen immediately — bypasses daily cap
      </p>
    </div>
  )
}
